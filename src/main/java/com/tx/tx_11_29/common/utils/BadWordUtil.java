...
public class BadWordUtil {
     ...
     * @version 1.0
     */
    public static Set<String> getEnBadWord(String txt , int matchType){
        Set<String> sensitiveWordList = new HashSet<String>();
        ...
        for(int i = 0 ; i < txt.length() ; i++){
            ...
            if(length > 0){    //存在,加入list中
                if (i + length == txt.length()) {
                    sensitiveWordList.add(txt.substring(i, i+length));
                } else if (txt.charAt(i + length) == 32 || isSign(txt.charAt(i + length))) {
                    sensitiveWordList.add(txt.substring(i, i+length));
                }
                i = i + length - 1;    //减1的原因，是因为for会自增
            }
    ...
    public static boolean containEnBadWord(String txt,int matchType) {
        if (getEnBadWord(txt, matchType).isEmpty()) {
            return false;
        }
    ...
    /**
     * 完全匹配 比如敏感词‘hello’ 文本:'12hello1' 虽然含有hello，但是前后有其他的字母，不算是完全匹配 文本必须是:'hello xxx'才算是含有敏感词
     * @param txt
     * @param matchType
     * @return
     */
    public static Set<String> exactMatch(String txt, int matchType){
        Set<String> sensitiveWordList = new HashSet<String>();

        String[] wordArray = StringUtils.tokenizeToStringArray(txt, " ");
        for (String word : wordArray) {
            for(int i = 0 ; i < word.length() ; i++){
                int length = checkBadWord(word, i, matchType);    //判断是否包含敏感字符
                if(length > 0){    //存在,加入list中
                    if (i + length == word.length()) {
                        sensitiveWordList.add(word.substring(i, i+length));
                    }
                    i = i + length - 1;    //减1的原因，是因为for会自增
                } else {
                    break;
    ...
    public static void main(String[] args) {
      /*  Set<String> s = BadWordUtil.words;
        ...
        Set<String> set2 = BadWordUtil.getBadWord(string, 2);
        ...
        //addBadWordToHashMap(Sets.newHashSet("hello123 t"));

     //   System.out.println(BadWordUtil.getBadWord("5hello123 t", 1));
